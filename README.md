...
`.vscode/` 에 프로젝트 공용 설정이 포함되어 있다.

- **추천 확장** (`.vscode/extensions.json`) — VS Code 로 프로젝트를 열면 우측 하단에 설치 알림이 뜬다 (강제 아님).

  | 확장                        | 용도                             |
  | :-------------------------- | :------------------------------- |
  | `tauri-apps.tauri-vscode`   | Tauri 설정·command 지원          |
  | `rust-lang.rust-analyzer`   | Rust 코드 인텔리센스             |
  | `bradlc.vscode-tailwindcss` | Tailwind v4 클래스·`@theme` 인식 |
  | `dbaeumer.vscode-eslint`    | ESLint(flat config) 연동         |
  | `esbenp.prettier-vscode`    | Prettier 포맷                    |

- **워크스페이스 설정** (`.vscode/settings.json`) — Tailwind v4 의 `@theme` 등 unknown at-rule 경고를 끈다. 프로젝트 정합성 키만 담고 있어 개인(User) 설정은 건드리지 않는다.

> 다른 에디터를 쓴다면: ESLint·Prettier·Tailwind LSP·rust-analyzer 연동 플러그인을 각자 설치하고, CSS 검사기에서 unknown at-rule 경고를 끄면 동일하게 동작한다.
