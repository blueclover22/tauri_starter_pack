...
export const appApi = {
  ping: (note?: string): Promise<{ message: string; echoedNote?: string }> =>
    invokeTauri("app_ping", { request: { note } }),
};
```
...
비민감 설정 영속화는 `tauri-plugin-store` 를 사용한다 (`shared/persistence/store.rs`). store 파일명·key 상수는 `shared/config.rs` 에서 관리하고, 접근은 helper 함수를 통해 수행한다 (command/service 에서 `StoreExt::store()` 직접 호출 금지).
